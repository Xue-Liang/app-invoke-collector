...
public abstract class AbstractGraphDao {
    ...
    protected static final RowMapper<Graph> mapper = new BeanPropertyRowMapper<Graph>(Graph.class);

    protected static final ResultSetExtractor<Graph> extractor = new ResultSetExtractor<Graph>() {
        @Override
        public Graph extractData(ResultSet rs) throws SQLException, DataAccessException {
            Graph row = new Graph();
            row.setId(rs.getLong("id"));
            row.setCounterId(rs.getInt("counter_id"));
            row.setValue(rs.getLong("value"));
            row.setInterval(rs.getInt("interval"));
    ...
    private static final String insert = "insert into graph (counter_id,value,interval,create_time,update_time)values(?,?,?,?,?)";
    ...
    public int update(Graph entity) {
        ...
        sql.set("interval", entity.getInterval());
    ...
    private static final String queryFirst = "select id,counter_id,value,interval,create_time,update_time from graph  where  id=? ";
